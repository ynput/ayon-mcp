"""Generate MCP tool wrappers from ayon_openapi.json.

The generated package lives in:
    ayon_mcp/tools/openapi_generated/

Design goals:
- Keep generated code isolated and removable.
- Split files by endpoint group (first segment after /api).
- Use RestApiClient for all generated requests.
"""
...
def _choose_body_content_type(
        request_body: dict[str, Any] | None) -> str | None:
    if not request_body:
        return None
...
def _main() -> None:
    ...
    output_dir = project_root / "ayon_mcp" / "tools" / "openapi_generated"
    ...
    (output_dir / "_runtime.py").write_text(
        _runtime_source(), encoding="utf-8"
    )
