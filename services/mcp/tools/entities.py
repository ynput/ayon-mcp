...
from ..utils import collect, entity_fields

if TYPE_CHECKING:
    import ayon_api
    from ayon_api.typing import ProjectHierarchyDict
...
def register(mcp: FastMCP, api: ayon_api.ServerAPI) -> None:
    """Register entity tools on *mcp* using *api* for data access.
    ...
    def get_folder_hierarchy(
        ...
        Returns:
            A nested dict of folders, each with a list of child folders and
            a list of task names. Each folder dict has keys:
                - "id": Folder id (hex string).
                - "name": Folder name.
    ...
    def list_folders(
        ...
        *,
        ...
        Returns:
            A dict with "count" and "items" keys. Each item is a folder dict
            with keys: "id", "name", "label", "path", "folderType",
            "parentId", "status", "tags", "active". If `include_attrib` is
            True, the dict also has an "attrib" key with attribute values.
    ...
    def get_entity(
        ...
        """
        getters = {
            "folder": api.get_folder_by_id,
            "task": api.get_task_by_id,
            "product": api.get_product_by_id,
            "version": api.get_version_by_id,
            "representation": api.get_representation_by_id,
        }
